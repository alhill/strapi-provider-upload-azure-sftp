...
const fixRoute = (basePath, file) => {
    const folderArr = basePath.split("/").filter(e => e)
    return [...folderArr, file].reduce((a, b) => a + "/" + b, "")
}
        ...
        return {
            upload: async file => {
                ...
                if(!private){
                    ...
                    try {
                        sftpRes = await sftp.put(file.buffer, fixRoute(basePath, fileName))
                    } catch (e) {
                        console.error(e);
                        sftpRes = e
                    }
